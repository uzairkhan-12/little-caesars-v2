...
export const Route = createFileRoute("/branch")({ component: Home });
    ...
    <Shell title="Branch" subtitle="Live devices, occupancy and today’s energy on the floor.">
      ...
      {energyOverview.data && (
        <section className="mt-10">
          <HomeEnergySummary overview={energyOverview.data} isAdmin={gate.data?.role === "admin"} />
        </section>
      )}
...
function UsageChangeCard({
  ...
  return (
    <div className="rounded-xl bg-card border border-border px-4 py-3">
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
      <div className={`mt-1 flex items-center gap-1.5 font-semibold ${change.tone}`}>
      ...
      <div className="text-sm tabular-nums mt-1">{formatHomeKwh(compare.current)} kWh</div>
      <div className="text-xs text-muted-foreground mt-0.5">
        {change.delta == null
          ? vs
          : `${change.delta > 0 ? "+" : ""}${formatHomeKwh(change.delta)} kWh ${vs}`}
      </div>
    </div>
  );
}

function HomeEnergySummary({
  overview,
  isAdmin,
}: {
  overview: {
    demo: boolean;
    todayKey: string;
    today: { total: number; dayKey: string };
    week: EnergyCompareLite;
    month: EnergyCompareLite;
    categories: Array<{ id: ReportTable; today: number }>;
  };
  isAdmin: boolean;
}) {
